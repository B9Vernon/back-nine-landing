...
ACTION), CAMERA, ACTIVE REFERENCES, START FRAME, PHYSICS, LIGHTING, POSITIVE LOCKS,
SINGLE-GOLF-BALL LOCK, VISIBLE IMPACT LOCK (hitting scenes), REFERENCE FIDELITY LOCK (whenever
image references are attached), the single AVOID line, and the final frame (ENDING CONTINUITY
FRAME/FINAL VISUAL BEAT). Delete every other section unless it materially helps. Full guidance:
...
the engine does not remember prior clips). This is the default, token-efficient scaffold; expand
it only in Mode N — maximum detail (`references/token-efficient-production-mode.md`).

**Reliability rules govern the whole prompt** (`references/generation-reliability.md`): describe
the correct state positively; keep the total under ~250 words with locks ≈30% of it; one primary
action per clip; one swing-phase window per clip with club orientation stated; ONE `AVOID:` line
of ~25 words max — never a stack of negative-lock paragraphs.
...
club(s), single ball, logo — within the engine's reference cap]
START FRAME: [exact image to load as the engine's start frame — the facility photo (e.g.
Bay1Asset1) for a first clip, the previous clip's final frame for every subsequent clip]
...
(hands on grip, clubhead at ground end), location geometry, screen placement, branding — all
identical; positive phrasing]
SINGLE-GOLF-BALL LOCK: [compact positive wording from references/single-golf-ball-protocol.md]
VISIBLE IMPACT LOCK: [hitting scenes only — compact positive wording from
references/club-ball-impact.md]
REFERENCE FIDELITY LOCK: [whenever image references are attached — compact — from
references/reference-image-fidelity.md]
AVOID: [ONE line, ~25 words max, only the failures at risk in this clip — generic categories,
hero objects named at most once — references/distortion-prevention.md]
